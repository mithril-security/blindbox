...
def download(args):
    ...
    if args.registry == "huggingface":
        snapshot_download(repo_id=args.name, cache_dir=tmp_dir, ignore_patterns=["*.tflite", "*.mlmodel", "*.msgpack", "*.safetensors", "*.ot", "*.h5"])
...
def load_from_store(name: str, cls: Type[PreTrainedModel], host: str = "127.0.0.1", port: int = 8000, force_download: bool = False) -> PreTrainedModel:
    ...
    if force_download or not os.path.exists(f"{model_store_dir}/models--{url_name}"):
        if not os.path.exists(model_store_dir):
            os.mkdir(model_store_dir)

        tmp_file = f"/tmp/model-store-{uuid4()}.tar"
        with open(tmp_file, "wb") as f:
            f.write(requests.get(f"http://{host}:{port}/model_store/{url_name}").content)
...
if __name__ == "__main__":
    ...
    download_parser.add_argument("--registry", choices=["huggingface"], default="huggingface", help="Model registry from which to download the model")
    download_parser.add_argument("name", help="Model name")
    download_parser.set_defaults(handler=download)

    remove_parser = subparsers.add_parser("remove", help="Download a model into the store")
